...
public class AssignmentTest {
  ...
  public void test_Assignment_valid() throws DIbuggerLogicException {
      ...
      for (int k = 0; k < 1000; k++) {
          ...
          assertEquals(s.getValueOf("x").toString(),(Integer.valueOf(3 + k).toString()));
          assertEquals(result.size(),1);
          assertEquals(result.get(0).getValueOf("x").toString(),(Integer.valueOf(3 + k).toString()));
      }
      ...
      for (int k = 0; k < 1000; k++) {
          ...
          List<TraceState> result = assign.run(); // run it
          assertEquals (s.getValueOf("y").toString(), (Integer.valueOf(3 * k).toString()));
          assertEquals(result.size(), 1);
          assertEquals(result.get(0).getValueOf("y").toString(), (Integer.valueOf(3 * k).toString()));
      }
  }
